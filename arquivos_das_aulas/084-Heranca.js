...
const novo = function(funcao, ...params) { //Perceba que o nosso new recebe o nome de "novo", e ele recebe uma função como parâmetro e também um campo de conjunto de parâmetros com a ajuda o operador spread "..." - O operador spread é um operador que recebe vários argumentos e os junta em um único argumento, esses argumentos ficam armazenados em um array...
    const objeto = {} //Nossa função cria um objeto vazio...
    objeto.__proto__ = funcao.prototype //Esse objeto recebe como __proto__ de referência o prototype da função que for passada como parâmetros, seja ela qual for...
    funcao.apply(objeto, params) //Ao final, nós aplicamos á função passada como parâmetro o objeto vazio que nós acabamos de criar e qualquer quantidade de parâmetros que esse objeto precise serão passados ao spread por através do argumento "params". E usamos também o método "apply()", esse método é usado sempre que desejamos aplicar uma função construtora - que contenha o elemento "this" - a um determinado objeto, e também aplicamos um array - geralmente armazenado numa variável como é o caso aqui - onde o 1º argumento do apply é o objeto que iremos instanciar e o 2º argumento é a variável que contém o array de argumentos da função...
    return objeto //Por fim, retornamos o objeto em si, que virá já instanciado...
}

const aula2 = novo(aula, 'Até breve', 4567) //Perceba que a forma de usar nossa função new criada e a função new convencional é praticamente a mesma, com a excessão de que o nome da função construtora vai dentro dos parâmetros da nossa função new criada...
